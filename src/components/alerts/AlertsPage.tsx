...
export default function AlertsPage() {
  const advancedEnabled = process.env.NEXT_PUBLIC_ALERTS_ADVANCED === "true";
      ...
      <div className="mx-auto max-w-4xl py-8">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          ...
          <div className="flex flex-col items-start gap-3 sm:items-end">
            ...
            {/**
             * Demo-suppression: Auto-fire and Subscribe are gated behind NEXT_PUBLIC_ALERTS_ADVANCED.
             * Restore for full release by flipping the env var to "true" in Vercel project settings.
             * This is suppression, not deletion — buttons remain wired for future restoration.
             */}
            {advancedEnabled ? (
              <>
                <button
                  type="button"
                  onClick={() => {
                    // Auto-fire wiring reserved for future restoration
                  }}
                  className="flex items-center gap-1.5 rounded-lg border border-glass-border px-3 py-1.5 text-xs font-medium text-atlas-text-secondary transition-colors hover:text-atlas-text"
                >
                  Auto-fire
