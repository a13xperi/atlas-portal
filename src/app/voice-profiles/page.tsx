...
export default function VoiceProfilesPage() {
  ...
  const handleSaveVoice = async (name: string, dimensions: VoiceDimensions) => {
    if (editorMode === "edit-personal") {
      ...
      await api.voice.createBlend(name, [
        { label: "Personal", percentage: 100 },
      ]);
    ...
    return (
      <AppShell>
        <div className="mx-auto max-w-4xl animate-pulse space-y-6 px-4 py-8">
          ...
          <div className="h-64 rounded-2xl bg-atlas-surface" />
        </div>
      </AppShell>
    );
  }
  ...
  return (
    <AppShell>
      <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6">
        ...
        <h1 className="mt-2 font-heading text-2xl font-bold tracking-tight text-atlas-text">
          Your Voices
        </h1>
        <p className="mt-1 text-sm text-atlas-text-secondary">
          Create and manage different voices for different contexts. Pick one when crafting.
        </p>
        ...
        <div className="mt-6 rounded-2xl border border-glass-border bg-atlas-surface p-6 sm:p-8">
          <div className="flex flex-wrap items-start justify-between gap-3">
            ...
            <div className="flex gap-2">
              ...
              <button
                type="button"
                onClick={() => setEditorMode(selectedIsPersonal ? "edit-personal" : "edit-blend")}
          ...
          {!selectedIsPersonal && selectedBlend && (
            <div className="mt-6">
              <p className="text-xs text-atlas-text-muted">
                This voice is a blend of {selectedBlend.voices.length} source{selectedBlend.voices.length !== 1 ? "s" : ""}. The AI mixes these voices when generating drafts.
              </p>
